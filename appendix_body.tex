...
executes iff $V\ge\tau$: welfare is $V$ on executed types and the floor
welfare $1$ otherwise. The worst case over $V\in[1,g]$ is therefore
balanced at $\tau=\sqrt g$, where the high-type loss $g/\tau$ and the
floor loss $\tau/1$ (just below threshold) both equal $\sqrt g$, and no
deterministic rule does better. Since $R_g$ embeds as a degenerate
single-minded subfamily of the full model, all three bounds
transfer.\hfill$\square$
\end{proof}
...
\emph{Lower bound (symmetrized swarm).} Fix orders $o_1,\dots,o_N$ with
floors $\varepsilon=L/(N\rho)$ and auxiliary orders $u_1,\dots,u_L$ with
floor $1$; let $G$ be the set of all orders, so $C_G\le2L$. Each identity
has the \emph{same} public option family $\{\{o_i\},G\}$; identities
...
$N\varepsilon\rho=L$, and fallback floors contribute at most $C_G\le2L$,
...
at most $(e/(e-1))g$. The pure-NE lower
