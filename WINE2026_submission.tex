...
cell admits $O(N\log g)$ and has a serial-demand $\Omega(\min(N,g))$
barrier. Online, the price splits: against realized
...
$j\in S$, a \emph{delivered score} $y_{i,S,j}\ge0$ in $j$'s requested
asset (trader-normalized units); the batch value is
$V_i(S)=\sum_{j\in S}y_{i,S,j}$, realized only if the batch executes.
Delivery is batch-specific: the same solver may achieve different
delivered scores for order $j$ in different batches (joint execution nets
flows and saves fees), so $V_i(S)$ is \emph{not} additive across orders
in general. The standalone value is $V_i(\{j\})=y_{i,\{j\},j}$, and the
complementarity factor (Definition~\ref{def:g}) measures exactly how far
batching lifts value above the sum of standalones. A winning bid delivers
$w_j\le y_{i,S,j}$ to each covered order; the solver's profit is
$V_i(S)-\sum_{j\in S}w_j$, kept in produced assets, not transferred
across traders. The floor is $c_j=\max_i V_i(\{j\})$. Reports are
...
$D=\sum_{j\in S}(c_j-y_{i,S,j})^+\le C_S$. Either drop the batch and settle
