...
We ask what price three design requirements carry, each answered in terms
of the same $g$ (with liquidity $\theta$). The main results are
Theorems~\ref{thm:pof}--\ref{prop:walras}; the same spread parameter then
governs incentives, online behavior, and benchmark manipulation.
...
instance is an asymmetry at the repair/drop indifference point; batch
size is not the parameter, and under a
...
be nonempty even when the transferable core is empty.
...
convex profit-curve argument; independent option pricing pays
$\Omega(g/\log^2 g)$, and the multi-agent multi-minded cell is pinned
between $O(N\log g)$ and an $\Omega(\min(N,g))$ barrier.
...
Fair winner determination is NP-hard but fixed-parameter tractable in the
number of directed asset pairs. Section~\ref{sec:empirics} calibrates
observable deployment diagnostics; the worst-case theory is independent
...
analysis. The broader intent-market and solver-competition setting has
also been modeled recently by Chitra et al.~\cite{chitra2024}, while
batch trading has been proposed as a response to AMM arbitrage, LVR, and
sandwich attacks~\cite{canidio2023}. Combinatorial auctions are
classical~\cite{cramton2006}, but
almost always \emph{with} a numeraire and quasilinear utilities; our
setting has neither, putting it closer in spirit to mechanism design
without money~\cite{procaccia2013} and recent no-transfer impossibility
phenomena in MEV-aware fee mechanisms~\cite{bahrani2024}. The
\emph{price of fairness} was introduced for
...
without transfers, and the sharp $g=1$ threshold should be contrasted
with the gross-substitutes frontier~\cite{kelso1982,gul1999}. Our
smoothness bound uses the first-price deviation of Syrgkanis and
Tardos~\cite{syrgkanis2013} and the robustness interpretation of
smoothness~\cite{roughgarden2015}; the online search bounds instantiate
one-way trading with a known spread~\cite{elyaniv2001}. We use recent
order-flow-auction models~\cite{ma2025,chitra2024} only for positioning:
our results are worst-case bounds for a no-numeraire fairness filter.
...
tractability. This is the order-level analogue of the uniform clearing
...
\end{theorem}
This isolates our game-theoretic statement from recent OFA models, which
study different strategic environments under proposer-builder
separation~\cite{ma2025,chitra2024}.
...
These numbers are not used to prove any theorem; they show that the
benchmark and spread parameters are operational rather than formal.
...
As $g\to1$ the bound collapses to $1$ (fairness becomes free), while the
visible-spread tail shows where non-trivial $g$ is operationally relevant.

\paragraph{Benchmark robustness.}
The same trace motivates the contamination model: under a naive local
hard-reserve benchmark, 97.1\% of auction-weighted cell-windows are
vulnerable in the supplementary diagnostic. We use this only as a design
signal for delayed, quantile-based, rate-limited, and inherited
benchmarks; the contamination parameters remain model primitives.
...
combinatorial auction. Two phenomena recur: no-numeraire feasibility
...
Two obstructions remain deliberately visible. The \emph{latent-batch}
obstruction blocks full-game smoothness because deviations can activate
batches absent under the original profile. The \emph{serial-demand
barrier} shows that report-independent service orders can lose
$\Omega(\min(N,g))$. Removing either seems to require a new idea.
